...
\begin{titlepage}
  ...
  \vspace{0.3cm}
  {\normalsize Versión del Manual 2.22 — 13 de Agosto de 2026\par}
  \vspace{0.5cm}
...
\begin{center}
  \small\textcolor{cGray}{%
    ...
    Manual de Usuario $\cdot$ v2.22 $\cdot$ 13 de Agosto de 2026\\[2pt]
    ...
    a los presets.\\[2pt]
    \textbf{Cambios v2.21} (31 de julio): \textbf{parches con espesor} (prisma) y
    \textbf{reglas de superposición} entre objetos de la escena.
    (A) \textbf{Parche como prisma}: se dibuja con el espesor real del tratamiento
    (10 cm por defecto) hacia el interior de la sala, con las aristas resaltadas
    para leerlo con cualquier color de relleno. Espesor 0 $\to$ plano, como antes.
    (B) \textbf{Espesor desde el material}: al elegir un material se autocompleta
    leyendo la construcción del nombre (suma capa absorbente $+$ cavidad $+$
    cámara de aire $+$ descuelgue; descarta la geometría en-plano de los paneles
    ranurados). Cubre 240 de 429 materiales; avisa si el espesor dibujado no
    coincide con el del $\alpha$ elegido, y muestra el $\lambda/4$.
    \emph{El espesor NO entra al solver}: el $\alpha(f)$ del catálogo ya se midió
    con ese espesor (misma lana, $\alpha$ a 63\,Hz cambia $\sim$15$\times$ entre
    20 y 100 mm) $\to$ sumarlo seria contarlo dos veces; y correr la frontera
    seria peor (a 34\,Hz un panel de 10 cm es $\lambda/100$: la onda lo atraviesa).
    Detalle en \S10.5.
    (C) \textbf{Fuente y receptor nunca dentro de un mueble}: el mueble se modela
    quitando el aire, asi que un punto ahi adentro evaluaba \textbf{NaN} y
    contaminaba toda la FRF sin dar error. Se bloquea en los dos sentidos (mover
    el punto al mueble, y poner el mueble sobre el punto). El receptor estaba
    especialmente expuesto por ser un punto sin bafle.
    (D) \textbf{Regla de solidos simetrica}: el bafle del parlante ahora tambien
    frena contra los muebles (antes solo frenaba el mueble contra el parlante,
    aunque el manual ya prometia lo contrario). Se mantiene el escape: lo que ya
    esta superpuesto se puede arrastrar para afuera.
    (E) \textbf{Aviso de mueble tapando un parche}: no bloquea (el prisma es
    dibujo), pero un mueble delante del absorbente lo tapa y su $\alpha$ efectivo
    baja respecto del catalogo.
    (F) \textbf{Fix: muebles y parches se mueven con el origen}: al cambiar la
    convencion de origen se trasladaban fuentes, receptor y puntos de escucha,
    pero muebles y parches se quedaban en el lugar viejo (el origen configurable
    es de v2.16; parches y muebles se agregaron despues).\\[2pt]
    \textbf{Cambios v2.22} (13 de agosto): \textbf{empaquetado del \texttt{.exe}
    puesto al dia} (seccion \S20 nueva).
    (A) \textbf{Bundle 490 MB mas liviano}: \texttt{build.bat} excluye lo que
    Anaconda arrastra y el proyecto no usa (Qt5WebEngineCore 107 MB, panel 101,
    botocore 92, llvmlite/numba 66, bokeh). Carpeta 1524 $\to$ \textbf{1032 MB},
    ZIP 570 $\to$ \textbf{414 MB}, sin perder ninguna funcion. Lo que queda es
    irreducible: \texttt{mkl\_*.dll} ($\sim$370 MB, BLAS de numpy/scipy con
    variantes de despacho por CPU), gmsh ($\sim$86 MB) y PyQt5 ($\sim$81 MB).
    (B) \textbf{El ZIP dejo de mentir su version}: el nombre estaba hardcodeado
    en \texttt{v2.12} y quedo congelado ocho versiones; ahora
    \texttt{pack\_distribution.py} lo lee del changelog de este manual.
    (C) \textbf{\S20 nueva $+$ verificador al dia}: flujo completo (compilar,
    verificar, smoke test, prueba visual, empaquetar), desglose del tamano y
    \emph{por que el ejecutable NO va al repositorio} (GitHub bloquea $>$100 MB
    y el historial queda inflado para siempre $\to$ la via correcta es GitHub
    Releases). El rango de \texttt{verify\_distribution.py} estaba en 100-800 MB
    y marcaba FAIL en un bundle correcto; ahora 700-1400 MB. Ademas
    \texttt{--collect-all} para trimesh y gmsh: no corregia un bug (PyInstaller
    ya los detectaba) pero los dos resuelven cosas en runtime que el analisis
    estatico no ve.
    (D) \textbf{Fix critico: el \texttt{.exe} salia sin las DLLs de Qt}. Anaconda
    no guarda las DLLs de Qt junto a PyQt5 sino en
    \texttt{<env>\textbackslash Library\textbackslash bin\textbackslash} y con
    otro nombre (\texttt{Qt5Core\_conda.dll}); PyInstaller las busca recorriendo
    el PATH, asi que al compilar desde una consola sin el entorno conda activado
    \emph{las omitia en silencio} --- el build decia ``BUILD OK'' y el programa
    moria al abrirlo con \texttt{DLL load failed while importing QtCore}.
    \texttt{build.bat} ahora arma ese PATH por su cuenta. Los dos chequeos
    automaticos tampoco lo detectaban: \texttt{verify\_distribution.py} solo
    miraba que existiera la CARPETA \texttt{PyQt5/} (existia, con los
    \texttt{.pyd}; faltaban las DLLs) y \texttt{test\_distribution\_smoke.py}
    daba OK con ``el proceso sigue vivo 15 s'' --- pero un dialogo de excepcion
    tambien es un proceso vivo. Ahora el primero exige
    \texttt{Qt5Core/Qt5Gui/Qt5Widgets} y el segundo lee el titulo de las ventanas
    del proceso.%
  }
\end{center}
