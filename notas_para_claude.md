...
## 1j. Batch v2.47 (21 Sep 2026) — tanda UI/UX + coherencia npm/modos

Plan en `plan_mejoras_ux.md`, criterio en `ux_principios.md` (heurísticas Nielsen filtradas a app de escritorio PyQt5; NO instalar skills/plugins de UI, todos son para web). Orden acordado: 6, 9, 1, 8, 2, 5, luego atajos (3, 7) y luego la 4 (cámaras P/I/L, EN DISCUSIÓN, defaults ya acordados en el plan). Hechas y probadas por el usuario:

- **6 (bug) — puntos del campo 3D**: `Field3DItem`/`PressureField3D.update`/`update_signed` usaban `GLScatterPlotItem(size=7, pxMode=True)` = píxeles fijos → gigantes en sala chica, motas en CAD grande. Fix: `pxMode=False` + tamaño en METROS vía `_field_point_size_world(points)` = 0.6·(mín paso de la grilla por eje). En `acoustic_viewer.py`.
- **9 — letras X/Y/Z en el 3D**: `_add_axes` en `viewer.py` agrega `GLTextItem` por eje (color = su flecha) pasando la punta; `AXIS_X/Y/Z` más saturados. GLTextItem existe en pyqtgraph 0.14 (color = QColor).
- **1 — Ctrl+I importa directo**: `main.py` `_quick_import_cad` (usa `_load_cad_for_dialog(None)` + `_apply_cad_mesh(center=True)`); antes Ctrl+I abría el panel. El panel sigue por su botón.
- **8 — panel FEM 2 botones**: los 4 controles de mallado viven en un QDialog `_fem_cfg_dialog` («Configuración de FEM»); en el panel quedan «Configuración de FEM…» + «Estado:» (badge) JUSTO arriba de «Calcular modos (FEM)». Los widgets siguen siendo `self.*` (toda la lógica los lee igual). Dentro del diálogo: «Aplicar sugerencia» (carga npm recomendado) + «OK» (`_fem_config_ok` → flag + accept → `_open_fem_config` corre `_solve_fem` tras cerrar el modal).
- **2 — Auto/Manual campo 3D**: combo `combo_field3d_mode` (0=Auto). `_field3d_auto_on()` gobierna el auto-render por cambio de frecuencia (`combo_mode`→`_maybe_auto_field3d`) y por mover fuente (`_deferred_field_update`). Manual = solo Enter/botón.
- **5 — mini-planta en cortes**: `PlanMiniPreview(QWidget)` en `section_dialog.py`, al lado del canvas de perfil; `set_active(i, done)` desde `_load_wall`. Pared activa roja + número, hechas azul, resto gris.
- **3 — Alt+Enter borra el campo**: atajo global en `main.py` (`_acoustic_clear_field` → `AcousticPanel.trigger_clear_field`); reversible (Enter recalcula), no toca modos/solución.
- **7 — rotar fuente 90° con Ctrl+Alt + click derecho**: en `viewer.mousePressEvent`, branch `RightButton and alt and ctrl` (ANTES del "Ctrl+Der = colocar fuente", que también tiene Ctrl) → `_pick_source` → `sourceRotateRequested.emit(idx, 90.0)` (grados, ya suma a `orientation`). Sin fuente bajo el cursor no hace nada.
- **«?» de atajos**: botón arriba-derecha del `IsoViewer` (`_help_btn`); tooltip = `_shortcuts_html()` (un solo lugar con teclado + mouse), click = `_show_shortcuts_popup` (`QToolTip.showText`). Reposición en `_reposition_help_btn`.
- **4a — presets de cámara P/I/L**: botones abajo-derecha (`_cam_preset_row`, sobre `axis_indicator`). `_CAM_PRESETS` = (elevation, azimuth): planta (90,-90), iso (30,45), lateral (0,0). `_on_cam_preset_clicked`: re-clic del activo libera; otro fija+bloquea. Lock = `_cam_preset != None`; los DOS caminos de orbit (modo Rotar `_rotate_view_drag`, y botón central) llaman `_clear_camera_preset()` al arrastrar (destraba y orbita). `reset_camera` (tecla 0) también libera. Solo cambia ángulo (conserva distance/center).
- **4b — flechas de ciclo**: `_cam_arrow_left/right` (overlays QPushButton semitransparentes ◀▶ a los costados, centradas vertical). Visibles solo con preset lateral/iso (`_refresh_cam_arrows`). `_cam_arrow_clicked(±90)` gira `opts["azimuth"]` en la grilla canónica (lateral 0/90/180/270, iso 45/135/225/315). Se ocultan al destrabar (orbit/tecla 0). Si el usuario dice que giran al revés = cambiar el signo del delta. **PLAN UI/UX 100% CERRADO** (9 items + «?» + 4a + 4b).

**Coherencia npm/modos con perturbación (bug reportado con `pruebafem.room`):**
- **Item #2 (mover botón)**: config + estado arriba de calcular (ya en el 8).
- **Item #3 (auto npm doble recomendación)**: con perturbación, el f_S REAL (T30 por modo) > estimador Sabine. El auto ahora, en 1 click, dimensiona con Sabine (pasada 1) y si el f_S de perturbación supera la validez de malla **re-malla+re-resuelve UNA vez** (guard `_fem_auto_resolved`, floor `_fs_pert_floor`, sube Nº modos a Weyl(f_S), flag `_fem_needs_resolve` disparado al final de `_solve_fem`). Blindado: no re-pregunta CAD-no-sólido ni fuentes-afuera en el re-solve. Vale voxel (npm) y gmsh (h). Motor fijo = aviso manual.
- **Weyl vs válidos NO es contradicción**: Weyl es asintótico y SOBREESTIMA el conteo real. Causa raíz doble: (a) escalonamiento del voxel (f_max ≈ 0.95·f_S) y (b) Weyl sobreestima. Umbral de re-mallado/aviso pasó a **1.10** en frecuencia (absorbe el staircase, ≈1.33 en conteo). Label honesto vía `_update_weyl_validity_label(fs, f_max, V, S)`: si `fs ≤ f_max·1.10` → «N válidos hasta ~f_max: cubren f_S (Weyl estima M, sobreestima)»; si no → «faltan modos, subí npm». Corre SIEMPRE post-solve (`_solve_fem` tras el clip), en `compute_and_show_schroeder` si ya hay modos, y en `_post_solve_schroeder_coherence` (perturbación+materiales). GOTCHA: sin materiales asignados (`n_asig=0`) el f_S cae al fallback α=0.05 y `rt_src="sabine"` aunque el modelo sea perturbación → por eso la reconciliación NO puede colgar de `rt_src=="perturbacion+sabine"`.

Todo compila (`py_compile`) y el usuario probó en GUI todo (6/9/1/8/2/5/3/7 + «?» + 4a + 4b + FEM). **Plan UI/UX 100% cerrado.** Núcleo físico intacto (render/atajos/UI = presentación).
