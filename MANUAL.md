...
**CAD con columna (u otro hueco interior):** si el recinto tiene una columna (dos cuerpos cerrados: sala + columna), el mallador **gmsh** ahora la resuelve boundary-fitted: resta la columna del recinto (sala − columna, dejando el túnel) y malla conforme a la geometría real. Requiere el backend de boolean `manifold3d` (incluido en los requisitos). Si el boolean no está disponible, o si el recinto tiene superficies curvas problemáticas (p.ej. un techo en arco importado que gmsh no puede parametrizar), el cálculo cae automáticamente al mallador **voxel** (que talla el hueco por paridad de rayos, con frontera escalonada) y lo avisa en el estado del motor. El mallado también limpia caras degeneradas (área ~0) que antes hacían fallar a gmsh con «Singular matrix 3x3». El mallado boundary-fitted (gmsh) de techos curvos importados está en desarrollo; mientras tanto, un techo en arco se puede modelar con la geometría paramétrica de la app (pestaña Geometría, techo «Arco»), que sí se malla conforme.
...
En modo «Optimizar mis fuentes», además de «Evaluar» y «Optimizar», está **«Sugerir ubicaciones (desde cero)»**: propone dónde poner las fuentes partiendo de configuraciones típicas (mono, estéreo, estéreo ancho, subs a 1/4 y 3/4, esquina, estéreo pegado a la pared), las **rankea por el norte elegido** sobre el campo real de la sala, y con «Aplicar sugerencia» crea las fuentes en la elegida. Es la colocación desde cero que antes solo estaba en Predicción, ahora con el mismo criterio que el resto del panel (por ejemplo, con norte «Mínimo SBIR» gana el estéreo pegado a la pared, que saca el peine de banda). Por eso el modo **«Ubicación de fuentes»** de Predicción ahora muestra un aviso que redirige acá (sigue funcionando; el modo «Combinado» de Predicción, que optimiza forma y ubicación juntas, es exclusivo de esa pestaña).
